...
public class CreateFromShipment extends CreateFrom 
{
	...
	protected Vector<Vector<Object>> getOrderData (int C_Order_ID, boolean forInvoice)
	{
		...
		sql.append("SELECT "
				+ "CASE WHEN co.c_doctype_id='1000016' then l.QtyOrdered-SUM(COALESCE(m.Qty,0)) else l.QtyOrdered-SUM(COALESCE(l.qtydelivered,0)) end ,"			//	1
		...
		.append(" LEFT OUTER JOIN M_Product p ON (l.M_Product_ID=p.M_Product_ID)"
				...
				+ " LEFT OUTER JOIN c_order co on (co.c_order_id=l.c_order_id)"
		...
		sql.append(" WHERE l.C_Order_ID=? "			//	#1
				+ "GROUP BY l.QtyOrdered,co.c_doctype_id,CASE WHEN l.QtyOrdered=0 THEN 0 ELSE l.QtyEntered/l.QtyOrdered END, "
